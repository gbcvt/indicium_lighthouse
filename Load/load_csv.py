...
load_csv.py
-----------
Carrega os dados de todos os arquivos CSV (de uma pasta ou de um .zip)
direto nas tabelas do PostgreSQL, respeitando o schema criado na
Questao 2 (schema.sql).

Usa o comando COPY do proprio Postgres para ler o CSV e inserir os
dados sem nenhum tratamento: sem remover nulos, sem mexer em acento ou
caractere especial. O arquivo entra exatamente como esta.

Bibliotecas usadas:
  - psycopg2 (biblioteca externa, precisa instalar: pip install psycopg2-binary)
  - csv, os, re, sys, zipfile, tempfile, shutil (padrao do Python 3)
